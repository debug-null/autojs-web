  ...
  public static init() {
    ...
    WebSocketManager.getInstance().addClientRequestListeners(async (req) => {
      ...
      await DeviceModel.updateById(device.device_id, {
        connect_time: moment().format("YYYY-MM-DD HH:mm:ss"),
      });

      return { type: "device", extData: device };
    });

    WebSocketManager.getInstance().addClientStatusChangeListener(
      (client, status) => {
        if (status === "open" && client.type === "device") {
          WebSocketManager.getInstance().sendMessage(client, {
            type: "hello",
            data: { server_version: 2 },
          });
        }
      }
    );
