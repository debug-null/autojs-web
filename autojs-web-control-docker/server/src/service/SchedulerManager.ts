...
export class SchedulerManager {
  ...
  public async initJobFromDb() {
    ...
    for (let i in schedulers) {
      await SchedulerManager.getInstance().addCronJob(
        schedulers[i].scheduler_id
      );
    }
  }
  ...
  public async addCronJob(scheduler_id: number) {
    ...
    try {
      const job = new CronJob(
        scheduler.cron_time,
        this.onTick.bind(this, scheduler.scheduler_id)
      );
      ...
      logger.info(
        `定时任务${
          ...
          //@ts-ignore
        }添加成功，将于${job.nextDate().format("YY-MM-DD HH:mm")}执行`
      );
      job.start();
    } catch (error) {
  ...
  private async onTick(scheduler_id: number) {
    ...
    ScriptExecutor.getInstance().run(
      scheduler.device_ids,
      `${scheduler.scheduler_name}-${script.script_name}`,
      script.script
    );
